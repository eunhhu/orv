...
The primary benchmark uses local reference adapters through the shop/commerce
library surface:
...
Provider-backed Stripe/carrier package runs are separate advanced variants.
...
Production builds mirror this benchmark contract into `deploy/preflight.json` under `benchmark`, and the checked preflight command list includes `orv benchmark-prepare . --participants 2`, `orv benchmark-report .`, and `orv benchmark-report . --require-pass`. They also emit `deploy/benchmark-evidence.json`, a checked evidence template keyed to the same preflight hash, `deploy/participant-notes-template.md`, and generated smoke tests write `deploy/smoke-output.txt` on success with the checked graph/client/route/trace summary. The evidence artifact carries the automated gate, success criteria, time budget, and data-to-record fields so benchmark reports stay tied to the same deploy preflight that `orv verify-build` checks.

Before a human run, run `orv benchmark-prepare dist --participants 2` to seed participant rows and copy raw-notes files under `dist/deploy/evidence/`. The command output includes `recording_handoff`, a checklist of task fields, participant fields, observation fields, benchmark data-to-record items, success criteria, and the final `orv benchmark-report . --require-pass` command. After a human run, fill the recorded fields in `deploy/benchmark-evidence.json` and run `orv benchmark-report dist --require-pass` to turn elapsed task time, required observation data, generated smoke output, participant-run metadata, reviewer attestation, failure classification, and the 5-hour limit into a checked JSON report. The report parses `deploy/smoke-output.txt`, requires the generated pass, matching absolute build directory, HTTP(S) base URL, graph-contract, generated route-count match, and `trace_stream_requested=1` markers instead of trusting a manually typed "passed" string, requires any copied `smoke_test_output` evidence to match the retained generated smoke-output artifact when both are present, treats duplicate smoke-output marker fields as incomplete evidence, keeps the report incomplete until the minimum participant-run evidence is recorded, requires each recorded `raw_notes_artifact` to point to a retained non-empty relative file under the build directory, rejects raw notes that still contain generated timestamp/failure-classification placeholders, generated template instruction prose, or mismatched participant/run ids, requires `raw_notes_sha256` to match the retained raw-notes file bytes, emits `participant_raw_notes_artifacts[]` with path safety/check/retained/non-empty/template-filled/identity-matched/hash status for reviewer handoff, requires `human_evidence_review` to attest that raw notes, smoke output, participant identities, and the no-AI claim were inspected, requires `human_evidence_review.reviewed_at` to be strict UTC and not earlier than any valid non-empty `participant_runs[].completed_at` timestamp, requires `failure_classification.primary` whenever a task or participant run failed, and requires `failure_classification.notes` when the primary category is `other`.
...
Use [samples/shop-benchmark-evidence.sample.json](samples/shop-benchmark-evidence.sample.json) as a field-level example only. Real evidence must be recorded in the generated `deploy/benchmark-evidence.json`, preserve its generated preflight hash, keep recorded `run_id` and `participant_id` values unique, keep `started_at`/`completed_at` as strict UTC timestamps like `2026-05-18T09:00:00Z` with completion not earlier than start, keep `raw_notes_artifact` paths non-empty and forward-slash relative to `dist` without absolute paths, Windows drive paths, backslash paths, or `..` traversal, make each raw-notes file's `participant_id` and `run_id` appear exactly once and match the corresponding evidence row, replace generated raw-notes placeholders and remove generated template instruction prose before recording a run, fill `raw_notes_sha256` after the final retained note is written, fill `human_evidence_review` with the reviewer, strict UTC `reviewed_at` that is not earlier than any valid non-empty recorded `completed_at`, all review booleans set to `true`, and non-empty notes, and set `recording_status` to `"recorded"` only after replacing sample participant data with retained raw notes/output whose participant/run identity and SHA-256 match the evidence row.
...
- failure classification from the fixed category list: syntax, scaffold, compiler/runtime error, editor, documentation, deploy config, smoke contract, or other; `other` requires notes
