...
HTML 요소는 노드입니다. Tailwind 클래스는 head 안의 subtoken으로 사용됩니다 — `class=`가 필요 없습니다:
...
Tailwind 클래스와 문자열 리터럴은 한 줄에서 head segment로 공존할 수 있습니다. 가독성을 유지하세요 — 줄이 너무 길어지면 `define`으로 추출하는 것을 고려하세요.
...
컴포넌트는 `@children`을 통해 nested slot content를 받습니다:
...
`@children`은 DOM 전용 개념이 아닙니다. nested invocation block을 투영하는 일반 slot-content operator입니다.
