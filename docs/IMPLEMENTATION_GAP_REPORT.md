...
- `native-host/desktop-packaging.json`, `native-host/package-desktop-app.sh`, `Info.plist`, entitlements는 local `.app` bundle 생성, 기본 ad-hoc signing, Developer ID hardened-runtime signing opt-in 계약을 제공하고 generated packaging script/codesign verify probe를 통과함
- 남은 작업: notarization credential flow, richer source permission UX, cross-platform desktop container matrix
