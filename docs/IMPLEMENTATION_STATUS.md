...
상위 goal은 Codex goal tool에 active 상태로 남아 있지만, 실행 단위는 다음 완료 조건으로 관리한다. Goal tool objective를 직접 수정할 수 없을 때는 이 절이 operational source of truth다. G011이 닫히면 다음 goal은 실제 human benchmark evidence 수집 또는 M4+ product hardening 중 하나로 별도 승격한다.

Active goal string:

> G011은 M0-M3 reference MVP를 제품 claim 가능한 수준으로 좁혀 증명한다. 완료 조건은 fresh shop template smoke와 benchmark pre-human path가 재현되고, source-bundle/ProjectGraph/origin-map/runtime trace/reveal/editor/DAP가 같은 source identity를 보존하며, imported-file diagnostics와 source rehydration이 정확하고, benchmark human-evidence verifier가 recorded deploy/report 양쪽에서 같은 기준을 강제하고, compiler core가 provider/shop semantics 또는 runtime-heavy dependency를 소유하지 않는다는 focused regression이 커밋되는 것이다.

| Track | Code-owned 완료 조건 |
|-------|----------------------|
| Core graph spine | `verify-build`가 source-bundle, ProjectGraph, OriginMap, route/listen/response origin, graph edge/origin-link, trace version/kind drift를 field/path-oriented error로 거부한다 |
| Imported diagnostics/source rehydration | imported two-file fixture의 diagnostics가 `span.file` 기준 파일/라인을 보고하고, build 후 원본 entry/import 파일을 삭제해도 DAP `loadedSources`와 `source`가 source-bundle의 정확한 text/path/checksum을 반환한다 |
