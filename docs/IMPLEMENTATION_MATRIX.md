...
| `orv reveal` / editor reveal payload | implemented | experimental | M3 | `orv-cli`, `orv-compiler` | reveal CLI tests, route/html/db/commerce/trace black-box integration test | `orv reveal`, `orv editor reveal` | Build origin to source/production payload, including graph-contract, native-server/static artifact, and preflight smoke evidence summary counters |
