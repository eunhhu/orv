...
| Reference tree-walking runtime | implemented | stable-ish | M0 | `orv-runtime` | Runtime CLI v1 contract regression, runtime unit tests | `orv run` | Main execution path; foreground `@out` stdout and runtime failure CLI envelope are documented in `docs/contracts/RUNTIME_CLI_V1.md`; long-running server lifecycle remains covered by narrower server contracts |
...
| `@html` static render | implemented | stable-ish | M1 | `orv-runtime`, `orv-compiler`, `orv-cli` | HTML Render v1 contract regression, `fixtures/e2e/shopping_mall.orv` | `orv run`, `orv build`, `orv run-build` | Zero-runtime HTML page rendering, static `pages/index.html` build output, static bundle-plan target, and `run-build` stdout behavior are documented in `docs/contracts/HTML_RENDER_V1.md`; client reactive rendering remains under Client Bundle v1 |
| Client reactive bundle | artifact only | stable-ish | M4+ | `orv-compiler`, `orv-cli` | Client Bundle v1 contract regression, build artifact tests, generated smoke client gates | `orv build`, `orv verify-build` | Manifest/reactive plan/JS/WASM bootstrap; public manifest/reactive-plan/JS/WASM artifact graph, stable 16-hex hash fields, capabilities, blockers, and binding surfaces are documented in `docs/contracts/CLIENT_BUNDLE_V1.md`; full DOM diff roadmap |
...
| DAP bootstrap | implemented | stable-ish | M3 | `orv-cli`, `orv-runtime` | DAP Debug Session v1 contract regression, DAP stdio initialize contract regression, DAP CLI tests, editor debug runner tests | `orv dap serve`, `orv editor debug`, `orv editor run-debug` | Runtime frame/locals/debug control subsets; `orv dap serve --stdio` initialize capabilities and initialized event envelope plus public debug runner/result/panel JSON shapes are documented in `docs/contracts/DAP_DEBUG_SESSION_V1.md`; launch-time `loadedSources`/`source` snapshots carry imported source checksums into editor/native-host debug payloads; build-backed editor exports and build-dir `run-debug` carry production graph/preflight/native/static/client/smoke summary context into standalone debug runners and result panels; `orv:frame:N` instruction breakpoints verify against pseudo-instruction frames |
