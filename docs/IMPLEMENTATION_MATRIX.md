...
| Route origin header | implemented | experimental | M1/M3 | `orv-runtime`, `orv-compiler`, `orv-cli` | origin runtime tests, generated smoke header contract test | `orv run`, generated deploy smoke | Emits `x-orv-origin-id` and `x-orv-response-origin-id`; generated smoke verifies exact header values against server artifacts |
...
| Runtime trace JSON / trace stream | implemented | experimental | M3 | `orv-runtime`, `orv-cli` | request trace JSON contract test, editor trace tests, optional generated smoke trace-stream check | `orv editor trace`, `orv editor trace-stream` | Shared trace schema for reveal; root/frame public keys and primitive field types are regression-covered; editor trace also accepts optional DB operation and commerce adapter origin ids and expands them into source/production navigation |
